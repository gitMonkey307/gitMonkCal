...
public struct CreateEventIntent: AppIntent {
    ...
    @Parameter(title: "Event Title") public var title: String
    ...
    public func perform() async throws -> some IntentResult {
        ...
        do {
            ...
            try await manager.saveEvent(id: nil, title: title, start: startDate, end: endDate, isAllDay: false, location: nil, notes: "Created via gitMonk Interactive Siri", calendarID: defaultCalID, alarms: [], recurrenceType: .none)
            return .result(dialog: "Added \(title) to your calendar by gitMonk Interactive.")
        } catch { return .result(dialog: "Failed to access calendar. Please check gitMonk Interactive permissions.") }
    }
}
