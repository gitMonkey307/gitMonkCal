...
struct MonthDayCell: View {
    ...
    private var isWeekend: Bool {
        let day = Foundation.Calendar.current.component(.weekday, from: date)
        return day == 1 || day == 7
    }
        ...
        VStack(alignment: .trailing, spacing: 1) {
            HStack {
                ...
                Text(date.formatted(.dateTime.day()))
                    ...
                    .foregroundColor(Foundation.Calendar.current.isDateInToday(date) ? .white : (isWeekend ? .secondary : .primary))
                    .padding(4).background(Foundation.Calendar.current.isDateInToday(date) ? Color.blue : Color.clear).clipShape(Circle())
            }
