...
public struct EventEditView: View {
    ...
    public var body: some View {
        NavigationView {
            Form {
                templateSection; typeSection
                Section(header: Text("Details")) {
                    Group {
                        TextField("Title", text: $title).font(DesignSystem.Typography.header)
                        TextField("Location", text: $location); recentLocationsPicker
                    }
                }
                if isTask { Section("Priority") { Picker("Level", selection: $taskPriority) { Text("None").tag(0); Text("High").tag(1); Text("Med").tag(5); Text("Low").tag(9) }.pickerStyle(.segmented) } } else { colorSection }
                timeSection
    ...
    private func save() async {
        ...
        do {
            if isTask { try await viewModel.eventKitManager.saveTask(id: taskToEdit?.id, title: title, dueDate: startDate, notes: notes, listID: selectedID, priority: taskPriority) } else { try await viewModel.eventKitManager.saveEvent(id: eventToEdit?.id, title: title, start: startDate, end: endDate, isAllDay: isAllDay, location: location, notes: notes, calendarID: selectedID, alarms: alarms, recurrenceType: recurrenceType, customColorHex: colorHex) }
            await viewModel.refreshData(); dismiss()
        } catch { print("Save fail") }; isSaving = false
    }
    ...
    private func setupInitialState() {
        if let e = eventToEdit {
        ...
        } else if let t = taskToEdit { isTask = true; title = t.title; startDate = t.dueDate ?? Date(); notes = t.notes ?? ""; selectedID = t.listID; taskPriority = t.priority } else {
            ...
            if let target = initialDate { if let noon = Foundation.Calendar.current.date(bySettingHour: 12, minute: 0, second: 0, of: target) { startDate = noon; endDate = Foundation.Calendar.current.date(byAdding: .minute, value: viewModel.defaultDuration, to: noon) ?? noon } }
        }
    }
    ...
    @ViewBuilder private var colorSection: some View { Section("Color") { ColorPicker("Highlight Event", selection: $customColor) } }
