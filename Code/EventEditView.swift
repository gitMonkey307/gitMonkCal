...
public struct EventEditView: View {
    ...
    public var body: some View {
        NavigationView {
            Form {
                ...
                if isTask {
                    Section("Priority") { Picker("Level", selection: $taskPriority) { Text("None").tag(0); Text("High").tag(1); Text("Medium").tag(5); Text("Low").tag(9) }.pickerStyle(.segmented) }
                } else {
                    Section("Custom Highlight") { ColorPicker("Highlight Event", selection: $customColor) }
                }
                timeSection
                if !isTask { recurrenceSection; Section("Alarms") { Button("Add Alarm") { alarms.append(-900) }; ForEach(0..<alarms.count, id: \.self) { i in HStack { Text("\(Int(abs(alarms[i])/60))m before"); Spacer(); Button(role: .destructive) { alarms.remove(at: i) } label: { Image(systemName: "minus.circle") } } } } }
                calendarListSection
                notesSection
                actionSection
            }
    ...
    private func save() async {
        ...
        // FIXED: Using robust toHex converter from gitMonk Interactive pass
        ...
        Section {
            Group {
                if let e = eventToEdit {
                    // Feature: Safe Location-aware Sharing
                    let shareText = "Event: \(title)\nAt: \(location.isEmpty ? "TBD" : location)\nDate: \(startDate.formatted())"
                    ShareLink(item: shareText) { Label("Share Event", systemImage: "square.and.arrow.up") }
                    
                    if !location.isEmpty, let encoded = location.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed), let url = URL(string: "http://maps.apple.com/?q=\(encoded)") {
                        Button { UIApplication.shared.open(url) } label: { Label("Open in Maps", systemImage: "map") }
                    }
                }
